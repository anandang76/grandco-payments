...
$sql = "SELECT  deviceID, sensorID, zoneID,  email ,deviation ,result ,testType ,collectedTime  FROM `bumpTestReport` WHERE  collectedTime >= '$fromDate' AND collectedTime <= '$toDate'";
...
// if ($deviceID == ''){
//   $sql = "SELECT  deviceID, sensorID, zoneID,  email ,deviation ,result ,testType ,collectedTime  FROM `bumpTestReport` WHERE  collectedTime >= '$fromDate' AND collectedTime <= '$toDate' order by zoneID, collectedTime DESC, sensorID ";
// }else{
//   $sql = "SELECT  deviceID, sensorID,zoneID, email ,deviation ,result ,testType ,collectedTime  FROM `bumpTestReport` WHERE deviceID = '$deviceID' AND collectedTime >= '$fromDate' AND collectedTime <= '$toDate' order by zoneID, collectedTime DESC, sensorID ";
// }
// echo $sql;
...
while ($result = mysqli_fetch_assoc($getResult))
{
  ...
  if ($olderZoneID != $result['zoneID']) {
    ...
    $varHTML = getBumpTestRptHeader($deviceID, $fromDateIn, $toDateIn, $userEmail);
  }
  ...
  $dateObj1 = new DateTime($result['collectedTime']);
  $collectedTime = $dateObj1->format(REPORT_DATE_FORMAT);

  $varHTML .= '<tr style="background-color: '.$cellColor.';">
  <td style=" text-align: center;" width="15.2%" >'.$collectedTime.'</td>
    <td style=" text-align: center;" width="14.2%" >'.$deviceTag.'</td>
    <td style=" text-align: center;" width="14.2%" >'.$sensorTag.'</td>
    <td style=" text-align: center;" width="14.2%">'.$result['testType'].'</td>
    <td style=" text-align: center;" width="10%">'.number_format($result['deviation'], 2).'</td>
    <td style=" text-align: center;" width="14.2%">'.$result['result'].'</td>
    <td style=" text-align: center;" width="18.2%">'.$result['email'].'</td>
  </tr>';
...
$rnd = rand(100, 999);
$fileDisplayName = 'Bump Test Reports.pdf';
$pdfPath = BASE_FILE_PATH_REPORTS.'/bumpTestReports_'.$rnd.'.pdf';
...
if ($sendEmail == "YES"){
  ...
  sendReportEmails(BUMP_TEST_REPORT, $pdfPath, $fileDisplayName, $tag, $fromDateIn, $toDateIn, $userEmail);
...
header('Content-Disposition: attachment; filename="Bump Test Reports.pdf"');
