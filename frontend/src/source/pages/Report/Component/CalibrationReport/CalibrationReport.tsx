...
import { DownloadCalibrationReport, GetCalibrationReport, SendCalibrationReport } from '@/source/service/ReportsService';
import { DataTable, DataTableSortStatus } from 'mantine-datatable';
import sortBy from 'lodash/sortBy';
import IconCalendar from '@/components/Icon/IconCalendar';

const CalibrationReport = ({ Zone }: any) => {
    ...
    const HandleSubmit = async () => {
        ...
        let response = await GetCalibrationReport(data)
    ...
    const HandleDownload = async () => {
        ...
        if(user && user.email != ""){
            ...
            DownloadCalibrationReport(data);
        }
    }
    ...
    const HandleSend = async () => {
        ...
        if(user && user.email != ""){
            ...
            let response: any = await SendCalibrationReport(data);
                    ...
                    columns={[
                        {
                            ...
                            render: ({collectedDate}) => moment(collectedDate).format('DD-MM-YYYY HH:mm:ss'),
                            sortable: true
                        },
                        {
                            accessor: 'device',
                        ...
                        },
                        {
                            ...
                            // render: ({calibrationDueDate}: any) => calibrationDueDate?.split(' ')[0],
                            sortable: true
                        },
                        {
                            ...
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({calibratedDate}) => calibratedDate ? moment(calibratedDate).format('DD-MM-YYYY') : '',
                            // render: ({calibratedDate}: any) => calibratedDate?.split(' ')[0],
                            sortable: true
                        },
                        {
                            accessor: 'result',
                        ...
                        },
                        {
                            ...
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({nextCalibrationDueDate}) => nextCalibrationDueDate ? moment(nextCalibrationDueDate).format('DD-MM-YYYY') : '',
                            // render: ({nextCalibrationDueDate}: any) => nextCalibrationDueDate?.split(' ')[0],
                            sortable: true
                        }
                    ]}
