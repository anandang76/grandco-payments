...
import { DownloadLimitEditLogReport, GetLimitEditLogReport, SendLimitEditLogReport } from '@/source/service/ReportsService';
import { DataTable, DataTableSortStatus } from 'mantine-datatable';
import sortBy from 'lodash/sortBy';
import IconCalendar from '@/components/Icon/IconCalendar';

const LimitEditLogsReport = ({ Zone }: any) => {
    ...
    const HandleSubmit = async () => {
        ...
        let response = await GetLimitEditLogReport(data)
    ...
    const HandleDownload = async () => {
        ...
        if(user && user.email != ""){
            ...
            DownloadLimitEditLogReport(data);
        }
    }
    ...
    const HandleSend = async () => {
        ...
        if(user && user.email != ""){
            ...
            let response: any = await SendLimitEditLogReport(data);
            ...
            {rowData.length > 0 && <div className="datatables">
                <DataTable
                    ...
                    columns={[
                        ...
                        {
                            accessor: 'criticalMin',
                            title: 'Critical Min',
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ limits }) => {
                                limits = JSON.parse(limits)?.criticalMin;
                                let changedLimits = '';
                                if(limits){
                                    changedLimits = `OLD: ${limits?.old} and NEW: ${limits?.new}`;
                                }
                                return changedLimits;
                            }
                        },
                        {
                            accessor: 'criticalMax',
                            title: 'Critical Max',
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ limits }) => {
                                limits = JSON.parse(limits)?.criticalMax
                                let changedLimits = '';
                                if(limits){
                                    changedLimits = `OLD: ${limits?.old} and NEW: ${limits?.new}`;
                                }
                        ...
                        },
                        {
                            ...
                            cellsClassName: 'dark:text-white',
                            render: ({ limits }) => {
                                limits = JSON.parse(limits)?.warningMin
                                let changedLimits = '';
                                if(limits){
                                    changedLimits = `OLD: ${limits?.old} and NEW: ${limits?.new}`;
                                }
                        ...
                        },
                        {
                            ...
                            cellsClassName: 'dark:text-white',
                            render: ({ limits }) => {
                                limits = JSON.parse(limits)?.warningMax
                                let changedLimits = '';
                                if(limits){
                                    changedLimits = `OLD: ${limits?.old} and NEW: ${limits?.new}`;
                                }
                        ...
                        {
                            accessor: 'outOfRangeMin',
                            title: 'Out of Range Min',
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ limits }) => {
                                limits = JSON.parse(limits)?.outOfRangeMin
                                let changedLimits = '';
                                if(limits){
                                    changedLimits = `OLD: ${limits?.old} and NEW: ${limits?.new}`;
                                }
                        ...
                        {
                            accessor: 'outOfRangeMax',
                            title: 'Out of Range Max',
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ limits }) => {
                                limits = JSON.parse(limits)?.outOfRangeMax
                                let changedLimits = '';
                                if(limits){
                                    changedLimits = `OLD: ${limits?.old} and NEW: ${limits?.new}`;
                                }
...
export default LimitEditLogsReport;
