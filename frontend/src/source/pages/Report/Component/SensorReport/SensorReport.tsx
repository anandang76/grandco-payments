...
import { DownloadSensorReport, GetSensorReport, SendSensorReport } from '@/source/service/ReportsService';
import { DataTable, DataTableSortStatus } from 'mantine-datatable';
import sortBy from 'lodash/sortBy';
import IconCalendar from '@/components/Icon/IconCalendar';

const SensorReport = ({ Zone }: any) => {
    ...
    AllDevices.sort(function(a, b){
        let x = a.deviceName?.toLowerCase();
        let y = b.deviceName?.toLowerCase();
        if (x < y) {return -1;}
        if (x > y) {return 1;}
        return 0;
    });
    ...
    const [Device, setDevice] = useState<any>(AllDevices[0]);
    ...
    const HandleSubmit = async () => {
        let response = await GetSensorReport({
    ...
    const HandleSend = async () => {
        ...
        if(user && user.email != ""){
            let response: any = await SendSensorReport({
                fromDate: moment(FromDate).format('YYYY-MM-DD'),
                toDate: moment(ToDate).format('YYYY-MM-DD'),
                locationID: Zone.locationID,
                branchID: Zone.branchID,
    ...
    useEffect(() => {
        if(Zone != ""){
            ...
            setDevicesOptions(AllDevices);
            setDevice(AllDevices[0]);
        }
        setRowData([]);
        setInitialRecords([]);
    }, [Zone])
            ...
            {rowData.length > 0 && <div className="datatables">
                <DataTable
                    ...
                    columns={[
                        ...
                        {
                            accessor: 'minScaledVal',
                            title: 'MIN',
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ minScaledVal }) => Number(minScaledVal).toFixed(2),
                            sortable: true
                        },
                        {
                            accessor: 'maxScaledVal',
                            title: 'MAX',
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ maxScaledVal }) => Number(maxScaledVal).toFixed(2),
                            sortable: true
                        },
                        {
                            accessor: 'avgScaledVal',
                            title: 'AVG',
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ avgScaledVal }) => Number(avgScaledVal).toFixed(2),
                            sortable: true
                        },
                        {
                            accessor: 'info',
