...
export const AlertComponent = ({ AlertModal, setAlertModal, deviceID }: any) => {
    ...
    useEffect(() => {
        if(AlertModal){
            let currentAlert = Alerts.filter(alert => alert.deviceID == deviceID);
            setRowData(currentAlert.reverse());
            setInitialRecords(currentAlert);
            setLoading(false);
        }
    }, [deviceID, AlertModal, Alerts])
                                                                ...
                                                                <DataTable
                                                                    ...
                                                                    columns={[
                                                                        ...
                                                                        },
                                                                        {
                                                                            ...
                                                                            title: 'Status',
                                                                            sortable: true,
                                                                            cellsClassName: '!py-2 !text-center dark:text-white w-40',
                                                                            render: ((current: any) => {
                                                                                let currentAlert = GetAlertPriorityStatus(current.alertType);
                                                                                let bgColor = currentAlert.color;
                                                                                let btnName = currentAlert.label;

                                                                                return <button type="button" className={`p-1 rounded-3xl btn-sm w-28 cursor-default status-btn border font-bold text-white  border-gray-400 text-sm dark:text-white`}style={{ backgroundColor: bgColor, border: bgColor }}>
                                                                                    {btnName}
                                                                                </button>
                                                                            })
                                                                        },
