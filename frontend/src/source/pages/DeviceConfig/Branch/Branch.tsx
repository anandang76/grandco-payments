...
import { DeleteBranch, GetBranch } from '@/source/service/DeviceConfigService';
...
const Branch = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Device config - Branch'));
    });
    ...
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({ columnAccessor: 'branchName', direction: 'asc' });
    const [BreadCrumb, setBreadCrumb] = useState<Array<any>>([]);

    const [BranchForm, setBranchForm] = useState<boolean>(false);
    ...
    const Handlelocation = (location: any) => {
        let { id, locationID } = location;

        navigate(`/device-config/${locationID}/${id}`);
    }
    ...
    const GetData = async () => {
        ...
        if(response.data?.status == "success"){
            ...
            setInitialRecords(sortBy(response.data.data, 'branchName'));
            ...
            CustomToast('Unable to fetch branch', 'error');
        }
    ...
    const Delete = async (id: number) => {
        ...
        let response = await DeleteBranch({
            id: id
        });
    ...
    return (
        <div className='grid xl:grid-cols-3 md:grid-cols-2 gap-6 mb-6'>
            <div className="panel h-full xl:col-span-2 !p-3 rounded-xl">
                <div className='flex flex-row justify-between pb-2'>
                    <ul className="flex space-x-2 rtl:space-x-reverse text-lg px-2">
                        ...
                        {BreadCrumb.map((selected, index) => <li key={index} className={`dark:text-white before:content-['/'] ltr:before:mr-2 rtl:before:ml-2 text-black font-bold text-md hover:underline ${selected.click == false ? 'pointer-events-none' : 'cursor-pointer'}`}>
                        ...
                        columns={[
                            {
                                ...
                                title: 'Branch Name',
                                render: (current) => <div className='cursor-pointer' onClick={() => Handlelocation(current)}>{current.branchName}</div>,
                                sortable: true
                            },
                            {
                                accessor: 'action',
            ...
            <AddBranch
                OpenModal={BranchForm}
                setOpenModal={setBranchForm}
                LocationData={BranchData}
