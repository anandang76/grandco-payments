...
import { DeleteFloor, GetFloor } from '@/source/service/DeviceConfigService';
import IconPlus from '@/components/Icon/IconPlus';
import Map from '../../Dashboard/Component/Map/Map';
import themeConfig from '@/theme.config';
import AddFloor from './Component/AddFloor';
...
const Floor = () => {
    ...
    const { locationID, branchID, facilityID, buildingID } = params;

    useEffect(() => {
        dispatch(setPageTitle('Device config - Floor'));
    });
    ...
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({ columnAccessor: 'floorName', direction: 'asc' });
    ...
    const HandleBreadCrumb = (location: any) => {
        let { locationID, branchID, id } = location;
    ...
    const HandleFloor = (floor: any) => {
        let { locationID, branchID, facilityID, buildingID, id } = floor;
        navigate(`/device-config/${locationID}/${branchID}/${facilityID}/${buildingID}/${id}`);
    }
    ...
    const GetData = async () => {
        ...
        if(response.data?.status == "success"){
            ...
            setInitialRecords(sortBy(response.data.data, 'floorName'));
            ...
            CustomToast('Unable to fetch floor', 'error');
        }
    ...
    const Delete = async (id: number) => {
        let message: string = 'Unable to delete floor';
    ...
    return (
        <div className='grid xl:grid-cols-3 md:grid-cols-2 gap-6 mb-6'>
                        ...
                        columns={[
                            {
                                ...
                                title: 'Floor Name',
                                render: (current) => <div className='cursor-pointer' onClick={() => HandleFloor(current)}>{current.floorName}</div>,
                                sortable: true
                            },
                            {
                                accessor: 'action',
            ...
            <AddFloor
                OpenModal={FloorForm}
                setOpenModal={setFloorForm}
                LocationData={FloorData}
