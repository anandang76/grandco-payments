...
import { DeleteBuilding, GetBuilding } from '@/source/service/DeviceConfigService';
...
const Building = () => {
    ...
    const { locationID, branchID, facilityID } = params;

    useEffect(() => {
        dispatch(setPageTitle('Device config - Building'));
    });
    ...
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({ columnAccessor: 'buildingName', direction: 'asc' });
    const [BreadCrumb, setBreadCrumb] = useState<Array<any>>([]);

    const [BuildingForm, setBuildingForm] = useState<boolean>(false);
    ...
    const HandleBuilding = (building: any) => {
        let { locationID, branchID, facilityID, id } = building;
        navigate(`/device-config/${locationID}/${branchID}/${facilityID}/${id}`);
    }
    ...
    const GetData = async () => {
        ...
        if(response.data?.status == "success"){
            ...
            setInitialRecords(sortBy(response.data.data, 'buildingName'));
            ...
            CustomToast('Unable to fetch building', 'error');
        }
    ...
    const Delete = async (id: number) => {
        let message: string = 'Unable to delete building';
        let status: string = 'error';

        let response = await DeleteBuilding({
            id: id
        });
    ...
    return (
        <div className='grid xl:grid-cols-3 md:grid-cols-2 gap-6 mb-6'>
                        ...
                        columns={[
                            {
                                ...
                                title: 'Building Name',
                                render: (current) => <div className='cursor-pointer' onClick={() => HandleBuilding(current)}>{current.buildingName}</div>,
                                sortable: true
                            },
                            {
                                accessor: 'action',
            ...
            <AddBuilding
                OpenModal={BuildingForm}
                setOpenModal={setBuildingForm}
