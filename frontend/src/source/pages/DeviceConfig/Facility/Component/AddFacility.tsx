...
import { GetFacility, UpdateFacility, AddNewFacility, removeImage } from '@/source/service/DeviceConfigService';
...
const AddFacility = ({ OpenModal, setOpenModal, LocationData, MapDetails, OnSuccess }: any) => {
    ...
    const HandleFacilityImage = (e: any) => {
        let file = e.target.files[0];
        if(file.type == "image/png" || file.type == "image/jpeg"){
            setLocationImage(file);
            setFormData((prevData: any) => ({
                ...prevData,
                ['facilityImageURL']: URL.createObjectURL(file)
            }))
    ...
    const HandleValid = () => {
        ...
        if(formData.facilityName){
            if(formData.facilityName == "" || formData.facilityName.trim() == ""){
                error['facilityName'] = "Facility Name is required";
                isValid = false;
            }
        } else {
            error['facilityName'] = "Facility Name is required";
    ...
    const HandleAddLocation = async () => {
        if(HandleValid()){
            ...
            if(LocationImage != null){
                Data.append('facilityImage', LocationImage);
            }
            ...
            if(LocationData.edit){
                ...
                response = await UpdateFacility(LocationData.id, Data)
            } else {
                response = await AddNewFacility(Data);
            }
    ...
    const GetData = async () => {
        ...
        if(response.data?.status == "success"){
            let { facilityName, coordinates, image } = response.data.data;
            let latitude = "";
            let longitude = "";
            ...
            setFormData({
                ...
                latitude: latitude,
                longitude: longitude,
                facilityImageURL: image != null && `${BackendURL}${image}`
            });
        } else {
            CustomToast('Facility not found','error');
            HandleClose();
        }
    }

    const confirmDelete = async (id: any) => {
        let response = await removeImage('Facility', id);
    ...
    const deleteImage = async () => {
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure, you want to delete this facility image?',
                                    ...
                                    <div className="flex flex-col items-center">
                                        <form className="flex items-center gap-8 w-full">
                                            <div className='w-full'>
                                                <label htmlFor="facilityName" className='dark:text-white'>Facility Name *</label>
                                                <input
                                                    type="text"
                                                ...
                                                />
                                                {formError.facilityName && <div className='text-sm text-danger'>{formError.facilityName}</div>}
                                            </div>
                                            <div className='w-full'>
                                            ...
                                            <div className='w-full'>
                                                <label htmlFor="facilityImage" className='flex gap-2 items-center dark:text-white'>
                                                    Facility Image
                                                    {formData?.facilityImageURL && <>
                                                        <Tippy content="View">
                                                            <a href={formData?.facilityImageURL} target='_blank' className='text-xs'>
                                                                <IconEye />
                                                            </a>
                                                        </Tippy>
                                                ...
                                                {formError.facilityImage && <div className='text-sm text-danger'>{formError.facilityImage}</div>}
                                            </div>
                                        </form>
