...
import { DeleteZone, GetZone } from '@/source/service/DeviceConfigService';
...
const Zone = () => {
    ...
    const { locationID, branchID, facilityID, buildingID, floorID } = params;

    useEffect(() => {
        dispatch(setPageTitle('Device config - Zone'));
    });
    ...
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({ columnAccessor: 'zoneName', direction: 'asc' });
    const [BreadCrumb, setBreadCrumb] = useState<Array<any>>([]);

    const [ZoneForm, setZoneForm] = useState<boolean>(false);
    ...
    const HandleBreadCrumb = (location: any) => {
        let { locationID, branchID, facilityID, id } = location;
    ...
    const HandleZone = (zone: any) => {
        let { locationID, branchID, facilityID, buildingID, floorID, id } = zone;
        navigate(`/device-config/${locationID}/${branchID}/${facilityID}/${buildingID}/${floorID}/${id}`);
    }
    ...
    const GetData = async () => {
        ...
        if(response.data?.status == "success"){
            ...
            setInitialRecords(sortBy(response.data.data, 'zoneName'));
            let coordinates = response.data.locationDetails.filter((data: any) => data.coordinates);
            ...
            CustomToast('Unable to fetch zones', 'error');
        }
    ...
    const Delete = async (id: number) => {
        let message: string = 'Unable to delete zone';
        let status: string = 'error';

        let response = await DeleteZone({
            id: id
        });
                        ...
                        columns={[
                            {
                                ...
                                title: 'Zone Name',
                                render: (current) => <div className='cursor-pointer' onClick={() => HandleZone(current)}>{current.zoneName}</div>,
                                sortable: true
                            },
                            {
                                accessor: 'action',
