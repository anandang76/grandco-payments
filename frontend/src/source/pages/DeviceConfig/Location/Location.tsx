...
import { DeleteLocation, GetLocation } from '@/source/service/DeviceConfigService';
...
const Location = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Device config - Location'));
    });
    ...
    const [LocationForm, setLoationForm] = useState<boolean>(false);
    ...
    const GetData = async () => {
        ...
        if(response.data?.status == "success"){
            setInitialRecords(sortBy(response.data.data, 'locationName'));
            let coordinates = response.data.data.filter((data: any) => data.coordinates);
            ...
            CustomToast('Unable to fetch location', 'error');
        }
    ...
    const Delete = async (id: number) => {
        ...
        let response = await DeleteLocation({
            id: id
        });
    ...
    const HandleLocation = (location: any) => {
        navigate(`/device-config/${location.id}`)
    }
    ...
    return (
        <div className='grid xl:grid-cols-3 md:grid-cols-2 gap-6 mb-6'>
            <div className="panel h-full xl:col-span-2 !p-3 rounded-xl">
                <div className='flex flex-row justify-between pb-2'>
                    <ul className="flex space-x-2 rtl:space-x-reverse text-lg px-2">
                        <li className="dark:text-white text-black font-bold text-md hover:underline cursor-pointer">
                            <span>Location</span>
                        </li>
                    </ul>
                        ...
                        columns={[
                            {
                                ...
                                title: 'Location Name',
                                render: (current) => <div className='cursor-pointer' onClick={() => HandleLocation(current)}>{current.locationName}</div>,
                                sortable: true
                            },
                            {
                                accessor: 'action',
            ...
            <AddLocation
                OpenModal={LocationForm}
                setOpenModal={setLoationForm}
