...
import { GetSensorOutput, DeleteSensorOutput } from "@/source/service/DeviceManagementService";
import CustomToast from "@/helpers/CustomToast";
import { DeleteIcon, EditIcon } from "@/source/helpers/Icons";
import SensorOutputTypeForm from "../SensorOutputTypeForm/SensorOutputTypeForm";
import Swal from "sweetalert2";

const SensorOutputType = () => {
    ...
    const GetData = async () => {
        ...
        if(response.data?.status == "success"){
            ...
            CustomToast('Unable to fetch sensor output type', 'error');
        }
    }
    ...
    const ConfirmDelete = async (id: any) => {
        let message: string = 'Unable to delete sensor output';
        let status: string = 'error';

        let response = await DeleteSensorOutput({
            id: id
        });
    ...
    const HandleDelete = async (id: any) => {
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure, you want to delete this sensor output?',
    ...
    return (
        <div>
            <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 className="font-semibold text-lg dark:text-white">SENSOR OUTPUT TYPE</h5>
                {/* <div className="ltr:ml-auto rtl:mr-auto">
                    <button className="btn btn-primary gap-2 dark:text-white" type="button" onClick={HandleAdd}>
                        <IconPlus />
                        Add Sensor Output Type
                    </button>
                </div> */}
            </div>
...
export default SensorOutputType;
