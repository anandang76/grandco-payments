...
import { GetCommunicationConfig, DeleteCommunicationConfig } from "@/source/service/DeviceManagementService";
import CustomToast from "@/helpers/CustomToast";
import { DeleteIcon, EditIcon } from "@/source/helpers/Icons";
import CommunicationSetupForm from "../CommunicationSetupForm/CommunicationSetupForm";
...
const CommunicationSetup = () => {
    ...
    const ConfirmDelete = async (id: any) => {
        ...
        let response = await DeleteCommunicationConfig({
            id: id
        });
    ...
    return (
        <div>
            <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 className="font-semibold text-lg dark:text-white">COMMUNICATION SETUP</h5>
                <div className="ltr:ml-auto rtl:mr-auto">
                    <button className="btn btn-primary gap-2 dark:text-white" type="button" onClick={HandleAdd}>
                        <IconPlus />
                        Add Communication Setup
                    </button>
                </div>
            </div>
                ...
                <DataTable
                    ...
                    columns={[
                        ...
                        },
                        {
                            ...
                            titleClassName: 'dark-datatable-title-color',
                            cellsClassName: 'dark:text-white',
                            render: ({ isWifi, isGSM, isFTP }: any) => {
                                let configMethod: Array<string> = [];

                                if(isWifi == 1){
                                    configMethod.push('Wifi');
                                }

                                if(isGSM == 1){
                                    configMethod.push('GSM');
                                }

                                if(isFTP == 1){
                                    configMethod.push('FTP');
                                }

                                return configMethod.join(', ');
                            }
                        },
                        {
                            accessor: 'actions',
